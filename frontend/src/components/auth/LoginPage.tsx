...
interface LoginPageProps {
  onLogin: (user: { email: string; role: 'superadmin' | 'schooladmin' | 'teacher'; name: string; schoolName?: string }) => void;
}
...
export function LoginPage({ onLogin }: LoginPageProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Store token in localStorage as backend expects
      ...
      const normalizedRole = roleMap[response.user.role] || 'teacher';
      ...
      onLogin({
        ...
        schoolName: response.user.schoolName || undefined,
      });
    } catch (err) {
